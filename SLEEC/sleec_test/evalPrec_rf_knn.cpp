...
void eval_prec_k_mult(smat_t Y, smat_t Yt, double *KNN, long long n, long long l, long long nt, long long numneighbors, long long returnK, double *pred_val, double *val, int numThreads)
{
    
   #pragma omp parallel for num_threads(numThreads)
	for(long long dataIter = 0; dataIter < nt; dataIter ++)
	{   
        //double *prec = (double *)malloc(sizeof(double)*returnK);
        //memset(prec, 0, returnK*sizeof(double));
  ...
  //       for (int i = 0; i < returnK; i++)
		// {
		// 	result[i + dataIter*returnK] = prec[i]/(i+1);
		// }
...
{	
	...
	PRED = mxCreateDoubleMatrix(returnK, nt, mxREAL);
	pred_val = mxGetPr(PRED);
	VAL = mxCreateDoubleMatrix(returnK, nt, mxREAL);
	val = mxGetPr(VAL);
	
	eval_prec_k_mult(Y, Yt, KNN, n, l, nt, numneighbors, returnK, pred_val, val, numThreads);
}
